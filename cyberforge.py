...
def adaptive_site_analysis(url, scope):
    ...
    score,level=risk(findings); data.update(risk_score=score,risk=level)
    return save_report("adaptive_site_analysis",data)
...
def ssl_certificate_intelligence(url, scope):
    ...
    data["findings"]=findings; score,level=risk(findings); data.update(risk_score=score,risk=level)
...
def robots_sitemap_audit(url, scope):
    ...
    data={"tool":"CyberForge","version":VERSION,"timestamp":stamp(),"target":url,"results":results,"findings":findings}; score,level=risk(findings); data.update(risk_score=score,risk=level)
...
def redirect_security_audit(url, scope):
    ...
    data={"tool":"CyberForge","version":VERSION,"timestamp":stamp(),"target":url,"chain":chain,"final_url":current,"findings":findings}; score,level=risk(findings); data.update(risk_score=score,risk=level)
